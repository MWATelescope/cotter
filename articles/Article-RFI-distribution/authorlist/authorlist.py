...
if (options.windows):
    ...
    for author in outlist:
        ...
        print author.printout,

    print " "
    print "----------------- Institutes ---------------"

    instlistout = []
    for inst in outinstlist:
        instlistout.append([str(outinstlist[inst]), str(inpinstlist[inst])])

    for oi in sorted(instlistout,key=lambda x:float(x[0])):
        print oi[0]+oi[1]
else:
