...
With estimates of $\alpha$, $\beta$, $S_L$ and $S_U$, one has obtained a parametrization of the RFI distribution. As was shown in \S\ref{sec:histogram-noise}, the left-most point where the power-law distribution falls off is \editmark{$Ig/4\pi r^2$} -- assuming free space propagation for the moment -- and therefore \editmark{$S_L = Ig/4\pi r^2$}. For Hata's propagation model, the more generic solution \editmark{$S_L = Ig / 4\pi \zeta r^\eta$} is found. This value represents the apparent brightness of the \editmark{RFI sources} that are furthest away from the telescope. With a fully parametrized distribution of the effect of RFI sources, an empirical model for RFI effects can be made. Moreover, one can calculate $E(S_R)$, the expected apparent strength of RFI:
