...
% A distribution can be expressed either as the probability density function (PDF) or as cumulative density function (CDF). This work uses PDFs, because they are better suited for visualizing the parameters we are interested in, although the two distributions basically convey the same information.
...
\end{equation}
with $\eta = 4.49 - 0.655 \log_{10} h_b$, one finds the rate density function $f_p$ that considers propagation effects,
% S(r):=I*g/(r^alpha);
% assume(g>0,I>0,S>0);
...
\end{equation}
where $x_u$ and $y_u$ are independently drawn from a uniform distribution between 0 and 1. The result is an amplitude sample $S_{\textrm{RFI}}$, with $S_{\textrm{RFI}} \ge Ig/r_L^2$, that follows a power-law distribution. The corresponding formula for drawing a sample $S$ that is contaminated by both RFI and noise is
