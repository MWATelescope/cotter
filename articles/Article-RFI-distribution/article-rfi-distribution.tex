...
\textrm{real}(S) \leftarrow x_n + \frac{Ig \cos 2 \pi y_u}{x_u r_L^2}
\textrm{,\hspace{1mm} imag}(S) \leftarrow y_n + \frac{Ig \sin 2 \pi y_u}{x_u r_L^2},
\end{equation}
with $x_n \sim N(\mu=0; \sigma)$. The final amplitude sample $S_A$ can be calculated with 
...
For completeness, we also show the formula for sampling apparent source strengths that include surface propagation effects, which can be used for sampling realistic source amplitudes in simulations. This is calculated by inversion and scaling of the frequency density function in Eq.~\eqref{eq:frequency-density-with-propagation}, resulting in:
\begin{equation}
\textrm{real}(S_{\textrm{RFI}}) \leftarrow \frac{\zeta I g}{x_u^{\eta/2} r_L^\eta},
\textrm{imag}(S_{\textrm{RFI}}) \leftarrow \frac{\zeta I g}{x_u^{\eta/2} r_L^\eta}.
\end{equation}
...
\end{equation}
with $N$ the number of samples and $x_i$ for $0 < i \le N$ the samples drawn from a power law with lower bound $x_\textrm{min}$.
