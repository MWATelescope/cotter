...
We have implemented the $w$-stacking algorithm in the C++ language. Several steps are multi-threaded in this implementation: reading and writing; gridding from and to different $w$-layers; performing the Fast Fourier transforms; and minor Clean iterations (peak-finding and image subtraction). For the latter, intrinsics were used as well. Because of these optimizations, minor Cleaning iterations are so fast that performing the Clark Clean optimization \citep{clark-clean}, which consists of considering only a subset of pixels with a reduced PSF, is not so relevant. However, because the PSF varies for imaging with a non-zero w-value, major iterations are still required to fix the errors from using a constant PSF during minor iterations.
...
\caption{MWA imaging example of the Vela and Puppis A supernova remnants. Left: normal projection centred on Puppis A, right: zenith projection to reduce $w$-values, but recentred on Puppis A. TODO time difference and more discussion, replace with deeper images.}
...
\end{align}
In words, recentering an image involves accounting for the position shift during the $w$-correction and final scaling, and shifting the visibilities in phase by multiplication with $e^{2 \pi i \left(u\Delta l + v\Delta m\right)}$ prior to the imaging.

A recentred image is in the projection of the zenith tangent plane, and will need an additional regridding step before multiple snapshots can be added together. An example of the difference in projection is given in Fig.~\ref{fig:vela-projection-example}. A recentred image can be stored in a Flexible Image Transport System (FITS) file, which allows setting the centre of tangent projection with the \texttt{CRPIXi} keyword \citep{wcs-in-fits}. Common viewers such as Kvis and DS9 support such FITS files and display their coordinates correctly. TODO show difference.
