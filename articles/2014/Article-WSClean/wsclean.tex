...
\begin{abstract}
We present a new imager implementation that uses the $w$-stacking algorithm and can make use of the $w$-snapshot algorithm. On MWA data, we find it to be an order of magnitude faster than $w$-projection, as well as being capable of full-sky imaging at full resolution. The performance dependencies of CASA's $w$-projection and WSClean are analysed and analytical functions are derived that describe the required computing cost for both imagers. The difference in computing cost is mainly caused by the efficiency of our new imager on MWA's 27\degree field of view. We predict the computing costs for several other configurations and estimate that our imager is approximately a factor of 2--3 faster on other arrays. Especially imaging SKA observations will become challenging, for which we predict that our new algorithm is still at least 30 times slower than is budgetted. We find that the $w$-snapshot has only a minor benefit. The source code of our new imager is publicly released.
\end{abstract}
...
Another option exists for imaging MWA data: the Real-Time System (RTS, \citealt{rts-mwa}). This has been designed as an efficient calibration and imaging pipeline specifically for MWA data. It can use GPUs for improving efficiency, and uses a w-projection kernel during imaging to correct for the w-term. The kernel can be kept small, because the data is processed in snapshots that have been phase-centered to zenith. The RTS can not perform deconvolution as of yet.
...
\begin{align}\notag
\frac{I'(l,m)\left(w_e - w_0\right)}{\sqrt{1-l^2-m^2}} = \int\limits_{w_0}^{w_e} e^{2\pi i w(\sqrt{1-l^2-m^2}-1)} \times \\ \label{eq:wstacking}
...
\begin{align}\notag
\frac{I'(\hat{l}, \hat{m})\left(w_e - w_0\right)}{\sqrt{1-\hat{l}^2-\hat{m}^2}} = \int\limits_{w_0}^{w_e} e^{2\pi i w(\sqrt{1-\hat{l}^2-\hat{m}^2}-1)} \times \\
...
WSClean is faster in all tested cases. Both imagers have performed 5 major iterations for these results, and major iterations are dominating the computing time. We will look more closely at the difference in performance in the next section.
...
\end{equation}
where $\max |\Delta \mathbf{x}|$ is the maximum baseline length, $\max \Delta z$ is the maximum height difference between antennas and $\lambda$ is the wavelength. These functions follow the trend of the measurements well, with an absolute error of 14.7\% and 20.7\% for respectively the WSClean and CASA functions. (TODO $w_e$ is slightly different for CASA)
...
\begin{table*}
\caption{Predicted computational cost for configurations listed in Table~\ref{tbl:computational-cost-configurations}, based on multi-frequency synthesis with 5 major iterations observing for 1 hour at ZA=20\degree with 1 polarization. Predictions are for $w$-projection with CASA; $w$-stacking with WSClean; $w$-stacked snapshots with WSClean using optimal snapshot duration; and a hybrid between $w$-projection and $w$-stacking using CASA with optimal $\Delta w$. All have approximately equal accuracy.} \label{tbl:computational-cost-predictions}
\begin{tabular}{l|rrrr|r}
               & \multicolumn{4}{c|}{Predicted computing cost on test computer} & max \\
 Configuration & $w$-projection & $w$-stacking & $w$-snapshot & hybrid & FLOPS/float \\
 \hline
 GLEAM    &   67h 7m & 8h 31m &  8h 05m & 15h 18m & $6.9 \times 10^2$    \\
 EMU      &    5.2 d &  2.9 d &   2.9 d & 5.2 d   & $2.1 \times 10^4$      \\
 MSSS low &  2h 18m  & 0h 59m &  0h 58m & 2h 16m  & $3.5 \times 10^3$   \\
 MSSS high&  7h 16m  & 3h 52m &  3h 52m & 7h 16m  & $3.4 \times 10^3$   \\
 LOFAR NL &  50.6 d  &   10 d &   7.2 d & 15.9 d & $4.5 \times 10^3$ \\
 AARTFAAC &  71h 25m &29h 50m & 29h 50m & 51h 34m & $6.8 \times 10^2$   \\
 VLSS     &  0h 12m  & 0h 12m &  0h 02m &  0h 12m & $12.1 \times 10^3$     \\
 MeerKAT  &   10.8 d &  6.2 d &   6.2 d &  10.8 d & $6.8 \times 10^2$ \\
 SKA-P1 AA inner&7.2 d& 2.9 d &   2.9 d &   3.7 d & $3.3 \times 10^5$   \\
 SKA-P1 AA mid&100,996 d&57,605 d& 57,605 d & 99,402 d & $3.3 \times 10^5$ \\
\end{tabular}
\end{table*}
...
In \citet{ska-phase1-2010}, the SKA computing power is budgetted by assuming an imaging speed of $10^4$ FLOPS/float. Extrapolation of our results shows that the current imagers perform $30$--$70$ times slower than this on SKA data. It was suggested that $w$-snapshots would improve the imaging speed for the SKA situation \citep{widefield-imaging-ska-cornwell}, but our results do not confirm this. This is mainly because the primary beam of a SKA station is not very large (1.2\degree), which causes the $w$-terms to not have a significant effect on performance: the bulk of the time spent is on gridding the antialiasing kernels instead of the $w$-kernels. Clearly it remains challenging to perform widefield imaging with acceptable performance. Nevertheless, some optimisations could be made for the SKA. For example, a performance gain of a few factors can already be achieved by averaging shorter baselines to their maximum integration time and bandwidth resolution before imaging.
