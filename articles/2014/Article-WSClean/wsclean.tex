...
A recentred image is in the projection of the zenith tangent plane, and will need an additional regridding step before multiple snapshots can be added together.

Another method to shift an image, is by using the periodicity of the FFT function. Since sources outside the field of view will be aliased back into the field, the image can be regridded with the alias in the centre. This complicates gridding, because the antialiasing kernel needs to have a pass-band shape instead of a low-pass shape. Therefore, we chose to implement the former method.

TODO discuss output standard fits file by using CPIXij keyword \citet{wcs-in-fits}.
