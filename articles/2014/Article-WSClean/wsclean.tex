...
\end{equation}
where $\max |\Delta \mathbf{x}|$ is the maximum baseline length, $\max \Delta z$ is the maximum height difference between antennas and $\lambda$ is the wavelength.

\begin{table*}
\caption{Predicted imaging times for various surveys, based on multi-frequency synthesis with 5 major iterations observing for 1 hour at ZA=20\degree.  Predictions are for $w$-projection in CASA, $w$-stacking with WSClean and $w$-stacked 12$\times$5-min snapshots with WSClean, with approximately equal accuracy.}
\begin{tabular}{lrrrrrrrrrr|rrr}
 Survey   & $\lambda$ & FWHM  & Beams & Ant & Res & $\max |\Delta \mathbf{x}|$ & $\max \Delta z$ & int. t & BW & $\Delta \nu$ & $w$-proj & $w$-stack & $w$-snaps \\
          &           &       &       &     &     & (km)                       &                 & (s)    & (MHz) &  (kHz) &  &  & \\
\hline
 GLEAM    &  2 m   & 24.7\degree& 1    & 128 & 2'   & 2.9 & 5~m & 2 & 32 & 40     &14h 02m & 1h 52 m & 1h 23m \\
 EMU      & 20 cm  & 1\degree   & 36   &  36 & 10''  & 6 &  0~m &10 & 300 & 20    &19h 49m &14h 12 m & 14h 13m\\
 MSSS low &  5 m   & 9.8\degree & 5    &  20 & 100'' & 5 &      &10 & 16 & 16     & 2h 30m & 0h 11 m & 0h 11m\\
 MSSS high&  2 m   & 3.8\degree & 5    &  40 & 120'' & 5 &      &10 & 16 & 16     & 2h 31m & 0h 38 m & 0h 39m\\
 VLSS     & 4.1 m  & 14\degree  & 1    &  27 &  80'' &11.1& 0~m &10 & 1.56 & 12.2 & 0h 32m & 0h 12 m & 0h 02m\\
\end{tabular}
\end{table*}

\subsection{Optimal snapshot time}
The computing time of snapshot imaging can be expressed in either of the $w$-correction techniques. Snapshot imaging effectively removes the dependency on ZA from $w_e$. Given the snapshot time $\Delta t_\textrm{snapshot}$ and the total time $\Delta t_\textrm{total}$, the total time cost of inversion becomes
\begin{equation}
t(\Delta t_\textrm{snapshot}) = \frac{\Delta t_\textrm{total}}{\Delta t_\textrm{snapshot}} t_\textrm{imaging}(w'_e,N'_\textrm{vis},N_\textrm{freq},N_\textrm{pix}),
\end{equation}
with
\begin{equation}
w'_e = \frac{1}{\lambda}\max \Delta z \left(1.0 - \cos(\frac{1}{2}(\textrm{FOV} + \angle t_\textrm{snapshot})\right)
\end{equation}
and $N'_\textrm{vis} = \frac{N_\textrm{vis}\Delta t_\textrm{total}}{t_\textrm{snapshot}}$. This function can be minimized to find the optimal snapshot time. For WSClean with MWA parameters, we find this function does not reach its minimum within 24~h, suggesting that the optimal snapshot time is as long as possible. This implies that the change in in performance due to higher $w$-values because of Earth rotation during an individual snapshot is less significant than the cost of the additional FFTs. For $w$-projection in CASA we find an optimal snapshot time of 122~seconds.

\subsection{Optimal $w$-project/$w$-stacking combination}
