...
\author[A.~R.~Offringa et al.]{A.~R.~Offringa$^{1,2}$\thanks{\hspace{2cm}E-mail: \url{andre.offringa@anu.edu.au}},
...
\begin{abstract}
We present a new interferometric imager that uses the $w$-stacking algorithm and can make use of the $w$-snapshot algorithm. On MWA data, we find it to be an order of magnitude faster than $w$-projection, as well as being capable of full-sky imaging at full resolution and with full polarization correction. The performance dependencies of CASA's $w$-projection and WSClean are analysed and analytical functions are derived that describe the required computing cost for both imagers. The difference in computing cost is mainly caused by the efficiency of our new imager on MWA's 27\degree field of view. We predict the computing costs for several other configurations and estimate that our imager is approximately a factor of 2--3 faster on other arrays. Imaging SKA observations is challenging, as we estimate the computing cost of full SKA1 low imaging to be 60 PetaFLOPS with current techniques. We find that the $w$-snapshot algorithm does not further improve computing requirements. The source code of our new imager is publicly released.
\end{abstract}
...
TODO: add big-ass full-sky image
...
TODO: I just realized that the original definition of $w$-snapshots might be Natasha's way of fixing $w$-values in MWACS. In that case, these methods are new and it might be good to distinguish this.
