...
Based on the expected cost terms described in Sec.~\ref{sec:time-complexity}, we fit analytical functions to the CASA and WSClean measurements. Several functions with different free parameters were tested, and formulae with minimum amount of parameters are selected that still follow the trend of the measurements and have reasonably small errors. The measurements are weighted with the inverse standard deviation. The single outlying measurement of CASA in Fig.~\ref{fig:timing-nsamples} is removed.
...
\end{equation}
Parameter $w_e$ is the maximum $w$ value and is esimated with
\begin{equation}
